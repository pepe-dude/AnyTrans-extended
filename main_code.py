...
from ocr_engines import OCRWrapper
...
def convert_gold_boxes(gold_boxes):
    """Converts {x, y, w, h} format to [[x,y], [x+w,y], [x+w,y+h], [x,y+h]]"""
    ocr_style_boxes = []
    for box in gold_boxes:
        x, y, w, h = box["x"], box["y"], box["w"], box["h"]
        ocr_style_boxes.append([
            [x, y],
            [x + w, y],
            [x + w, y + h],
            [x, y + h],
...
def render_text_generative(pipe, input_data, response, return_box, evaluate_ocr, idx, temp_dir, max_attempts=5):
    '''
    ...
    for attempt in range(max_attempts):
        ...
        img_path = os.path.join(temp_dir, "inpaint.png")
        cv2.imwrite(img_path, image)

        crop_image_path = image_crop(return_box, img_path, idx)
        crop_result = evaluate_ocr.ocr(crop_image_path)

        if not crop_result or crop_result[0] is None:
            detected_text = ""
...
def PPOCR_pipline(
    ...
    trans_mode = f'{source_lang}2{target_lang}'

    # Prepare folders for the intermediate and final images
    ...
    if gold_data:
        all_text_ocr = gold_data['source_texts']
        dt_boxes = convert_gold_boxes(gold_data["source_text_bounding_boxes"])
    else:
    ...
    resize_image_path = os.path.join(temp_dir, f"{base_name}_resize.png")
    ...
    if log:
        evaluate_log = os.path.join(log_output_dir, f"{base_name}_trans_log.txt")
        evaluate_log_file = open(evaluate_log, "w", encoding="utf-8")

        # JSON logging
        json_log_data = {
            "source_text_bounding_boxes": [],
            "source_texts": all_text_ocr,
            "target_texts": translate_responses
        }
        for box in dt_boxes:
            box_np = np.array(box)
            ...
            json_log_data["source_text_bounding_boxes"].append({
                "x": int(min_x),
                "y": int(min_y),
                "w": int(max_x - min_x),
                "h": int(max_y - min_y)
        ...
        json_log_path = os.path.join(log_output_dir, f"{base_name}_metadata.json")
    ...
    for idx in range(len(new_dt_boxes)):
        ...
        resized_mask, return_box = resize_mask_returnbox(ori_image_path, box,char_count_ori, char_count_trans)
        resized_masked_image = cv2.bitwise_and(image, image, mask=resized_mask)

        # masked_image_path = img_path[:-4] + '_masked_'+str(idx) + '.png'
        resized_masked_image_path = os.path.join(temp_dir, f"{base_name}_resized_masked.png")
        ...
        if untranslate == False: #TODO: Perform only when rendering deterministic
            inpaint_image(ori_image_path, box)
        ...
        edited_image_path = os.path.join(output_dir, f"{base_name}_translated.png")
        if response == txts:
            ...
            if rendering == "generative":
                ...
                edited_image, detected_text = render_text_generative(pipe, input_data, response, return_box, evaluate_ocr, idx, temp_dir)                
                ...
                if log:
                    evaluate_log_file.write(f"{txts}\t{response}\t*deterministic*\n")

            cv2.imwrite(edited_image_path, edited_image)

        ori_image_path = edited_image_path
        image = edited_image.clip(1, 255)


def main(args: argparse.Namespace):
    ocr = OCRWrapper(args.ocr_engine, args.source_lang)
    evaluate_ocr = OCRWrapper(args.ocr_engine, args.target_lang)
    gold_data = None

    for filename in os.listdir(args.input_dir):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
            if args.use_gold:
                img_id = re.search(r'(\d+)', filename).group(1)
                parent_dir = os.path.dirname(args.input_dir)
                metadata_path = os.path.join(parent_dir, f"{img_id}_metadata.json")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--source_lang", default="cs", choices=["cs", "en", "uk", "german"])
    parser.add_argument("--target_lang", default="en", choices=["cs", "en", "uk", "german"])
    parser.add_argument("--ocr_engine", default="paddle", choices=["paddle", "tesseract", "easyocr"], help="OCR engine to use")
    parser.add_argument("--rendering", default="deterministic", choices=["generative", "deterministic"])
    parser.add_argument("--input_dir", type=str, default="/home/pepe/Documents/University year 4/Competing in machine translation/cs2en/images")
    parser.add_argument("--output_dir", type=str, default="/home/pepe/Documents/University year 4/Competing in machine translation/AnyTrans-extended/OCR_test_results")
    parser.add_argument("--log", type=bool, default=False)
    parser.add_argument("--use_gold", type=bool, default=False, help="Use gold source text from metadata")
    parser.add_argument("--box_by_box", type=bool, default=False, help="Pass the text boxes for translation one by one")
